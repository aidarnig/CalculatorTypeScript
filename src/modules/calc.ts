    ...
    return inputs.reduce<OperationsBuilder>(
        ...
        switch(input.type) {
            case InputType.Numerical:
                ...
                const newValue = prevValue * 10 + input.value; // построение одного числа из цифр
...
const getState = (inputs: Array<CalcInput>): CalcState => {    
    ...
    switch(lastOperation.operator) {
        case OperatorType.Equals:
            return {displayValue: total}; // возвращаем подсчитанное число
        
        default:
            return{displayValue: lastInput && lastInput.type === InputType.Numerical ? builder.working.value : total}; // возвращаем последнее записанное число
    }
};
